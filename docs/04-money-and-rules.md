...
| **ARTP** (telecom regulator) | USSD short codes, SMS sender IDs, voice/IVR numbers, M2M SIM arrangements. | Short code application via operators/aggregator early — lead times are long. A plain local voice number for the IVR status line (doc 01) works day one while the short code is pending. |
